...
class GLViewer:
    ...
    def update_bounding_boxes(self, _bboxes):
        """Update bounding boxes to be rendered"""
        self.mutex.acquire()
        self.bounding_boxes = _bboxes
        self.mutex.release()
    ...
    def draw_bounding_boxes(self):
        ...
        for bbox_data in self.bounding_boxes:
            ...
            # Set color (red for persons)
            bbox_color = [1.0, 0.0, 0.0]  # Red color for bounding boxes
            glUniform3fv(self.shader_color_loc, 1, bbox_color)
    ...
    def draw(self):  
        if self.available:
            ...
            # Draw bounding boxes
            if self.tracking_state == sl.POSITIONAL_TRACKING_STATE.OK and len(self.bounding_boxes) > 0:
